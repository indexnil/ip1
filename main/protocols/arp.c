...
static ipv4_addr_t self_ip_addr;
static mac_addr_t self_mac_addr;

// First 4 bytes are the IP, the latter 6 bytes are the MAC
uint8_t ip_mac_cache[IP_MAC_CACHE_LENGTH][10];

struct arp_pending_t arp_pending_tasks[MAX_PENDING_IPS];

QueueSetHandle_t arp_queue_set = NULL;
QueueHandle_t arp_request_queue = NULL;
QueueHandle_t arp_input_queue = NULL;
...
void add_ip_mac_cache_entry(struct arp_entry_t* entry){
    ...
    if (is_duplicate_found == 0){
        // Add a new ip mac entry to the cache
...
esp_err_t send_arp_request(uint8_t ip[4]){
...
void arp_input(struct arp_data_t* buffer, uint16_t len){
    ...
    if (ntohs(buffer->operation) == ARP_OP_REPLY){
        ...
        if (xResult != pdTRUE){
            // Only error path is errQUEUE_FULL
            ESP_LOGW(TAG, "Could not hand received reply to the arp worker task, the queue is full!");
        }
        return;
    }
    ...
    while (1){
        ...
        if (handle == arp_request_queue){
            if (xQueueReceive(arp_request_queue, &request, 0) == pdPASS){
                // The queue has an object to be processed
                ...
                if (p == NULL){
                    ...
                    esp_err_t result = send_arp_request(request.ip);
                    if (result != ESP_OK){
                        ESP_LOGW(TAG, "Unable to send a new arp request");
                        xTaskNotifyGive(request.task); // Simply continue the task to prevent freezing the program
                        continue;
                    }
                }
        ...
        } else if (handle == arp_input_queue){
            if (xQueueReceive(arp_input_queue, &entry, 0) == pdPASS){
...
void arp_init(void){
    ...
    if (arp_input_queue == NULL){
        arp_input_queue = xQueueCreate(4, sizeof(struct arp_entry_t));
        if (arp_input_queue == NULL) {
            ESP_LOGE(TAG, "Failed to create arp_input_queue");
            return;
        }
    }

    if (arp_queue_set == NULL){
        arp_queue_set = xQueueCreateSet(8);
        if (arp_queue_set) {
            // Only possible error is if these queues already belong to a set, which they dont
            xQueueAddToSet(arp_request_queue, arp_queue_set);
            xQueueAddToSet(arp_input_queue, arp_queue_set);
        } else {
            ESP_LOGE(TAG, "Failed to create arp_queue_set");
            return;
        }
    }
